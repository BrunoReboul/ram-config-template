...
fw_rule_is_restricted(fw_rule, params) {
	...
	except := [ip_config | ip_config := ip_configs[_]; not excempt(ip_config)]
	trace(sprintf("rule_name: %v", [fw_rule.name]))
	trace(sprintf("ip_configs: %v", [ip_configs]))
	trace(sprintf("except: %v", [except]))
	trace(sprintf("count: %v", [count(except)]))
	count(except)>0
	# check protocol and port
#	fw_rule_check_protocol_and_port(ip_configs, params.protocol, params.port)

	fw_rule_check_protocol_and_port(except, params.protocol, params.port)
...
excempt(ip_config) = result {
	trace(sprintf("ip_config: %v", [ip_config]))
	ip_config.ipProtocol == "icmp"
	result := {}
	}

excempt(ip_config) = result {
	trace(sprintf("ip_config: %v", [ip_config]))
	ip_config.ipProtocol == "tcp"
